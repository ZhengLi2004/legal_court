...
class GraphAdapter:
    ...
    def get_edge_config(source, target, edge_data):
        ...
        return Edge(
            source=source, target=target, label=e_type, color=color, type="CURVE_SMOOTH"
        )
    ...
    def parse_shadow_graph(graph_dict: dict):
        ...
        for n_id, n_data in G.nodes(data=True):
            nodes.append(GraphAdapter.get_node_config(n_data, n_id))

        for u, v, e_data in G.edges(data=True):
            edges.append(GraphAdapter.get_edge_config(u, v, e_data))

...
from actions.worker_actions import AnalyzeSearchResults, SuggestPivot
...
class LawWorker(BaseWorker):
    def __init__(self, name: str = "LawWorker", es_tool: LawEsTool = None, llm: GPTChat = None, threshold: float = 0.6): super().__init__(name, "Law Researcher", es_tool, llm, threshold)

    async def _perform_search(self, query: str) -> str: return await self.es_tool.search_laws(query)
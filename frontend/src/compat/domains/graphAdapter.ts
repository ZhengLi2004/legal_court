import { buildLocalGraphDiff, normalizeGraph, normalizeGraphDiff } from '../protocol'
import { withQuery } from '../client'
import type { CompatClient } from '../client'
import type { GraphAdapter, GraphDiffView, GraphView, SessionAdapter } from '../types'
...
export class GraphDomainAdapter implements GraphAdapter {
  ...
  async getGraphDiff(sessionId: string, fromRound: number, toRound: number): Promise<GraphDiffView> {
  ...
  private async getGraphAtRound(sessionId: string, round: number): Promise<GraphView> {

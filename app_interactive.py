...
from mas.common import NodeType, EdgeType
...
with st.sidebar:
    ...
    if not st.session_state.is_setup:
        ...
        if not engine.is_finished:
            ...
            if st.button(f"▶️ Run {turn_name}'s Turn", type=btn_color, use_container_width=True):
...
if st.session_state.is_setup:
    ...
    with col_context:
        ...
        if snapshot.get("shadow_graph"):
            ...
            c2.metric("Conflict Edges", stats.get('conflict_edges', 0), delta_color="off")
        ...
        st.markdown("---")
        tab_facts, tab_memory, tab_raw = st.tabs(["📂 Evidence Base", "🧠 Global Memory", "📝 Raw Logs"])
        ...
        with tab_facts:
            ...
            if snapshot.get("shadow_graph"):
                ...
                laws = [d["content"] for _, d in sg.nodes(data=True) if d.get("type") == NodeType.LAW]

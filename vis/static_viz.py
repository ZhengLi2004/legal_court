...
from mas.common import NodeStatus, ShadowGraph
...
class GMemoryVisualizer:
    ...
    def draw_shadow_graph(
        self,
        shadow_graph: ShadowGraph,
        filename: str = "shadow_graph.png",
        title: str = "",
    ):
        ...
        for n, d in G.nodes(data=True):
            ...
            elif str(status) == "DEFEATED":
                edgecolors.append("red")

            else:
                edgecolors.append("gray")  # Hypothetical
    ...
    def draw_query_graph(
        self, task_layer: TaskLayer, filename: str = "query_graph.png"
    ):
        G = task_layer.graph
        ...
        nx.draw_networkx_edges(
            G, pos, width=[w * 2 for w in weights], edge_color="gray", alpha=0.5
        )
    ...
    def draw_insight_graph(
        self, insights_manager: InsightsManager, filename: str = "insight_graph.png"
    ):
        ...
        for i, inst in enumerate(insights):
            G.add_node(i, content=inst.content, score=inst.score)

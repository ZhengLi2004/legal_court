...
function getGraphAtRound(session: MockSession, round: number): MockGraphSnapshot {
...
function computeDiff(session: MockSession, fromRound: number, toRound: number): Record<string, unknown> {
...
export class MockTransport implements CompatTransport {
  ...
  async request<T = unknown>(req: TransportRequest): Promise<T> {
    ...
    if (method === 'POST' && (pathname === '/sessions' || pathname === '/engine/init' || pathname === '/api/v1/sessions')) {
    ...
    if (method === 'GET' && (pathname === '/sessions' || pathname === '/api/v1/sessions')) {
      return {
        sessions: [...this.sessions.values()].map((session) => toSnapshotResponse(cloneSession(session))),
      } as T
    }
    ...
    if (match) {
      ...
      if (method === 'GET' && action === 'graph') {
        const graph = getGraphAtRound(session, Number.isFinite(roundQuery) ? roundQuery : session.current_round)
        return {
          session_id: session.session_id,
      ...
      if (method === 'GET' && action.startsWith('snapshots/')) {
        ...
        const graph = getGraphAtRound(session, Number.isFinite(round) ? round : session.current_round)
        return {
          session_id: session.session_id,
          round_idx: graph.round,
          graph_data: {
      ...
      if (method === 'GET' && action === 'memory') {
        return {
          ...
          dynamic_law_case_count: Math.max(2, Math.floor(session.current_round / 2) + 1),
          task_layer: {
      ...
      if (method === 'GET' && (action === 'events' || action.startsWith('events/history'))) {
      ...
      if (method === 'GET' && action === 'diff') {
        ...
        const fromRound = Number.isFinite(fromRoundRaw) ? fromRoundRaw : Math.max(session.current_round - 1, 0)
        const toRound = Number.isFinite(toRoundRaw) ? toRoundRaw : session.current_round
  ...
  private pushSnapshot(session: MockSession): void {
    const existing = session.snapshots.find((item) => item.round === session.current_round)
    if (!existing) {
      session.snapshots.push(buildGraph(session.current_round))
    }
    const graph = getGraphAtRound(session, session.current_round)
    session.metrics.arguments = graph.nodes.length
    session.metrics.attacks = graph.edges.filter((edge) => edge.type === 'ATTACK').length
    session.metrics.supports = graph.edges.filter((edge) => edge.type === 'SUPPORT').length
  }
  ...
  private advance(session: MockSession): void {
    ...
    session.transcript.push(`[plaintiff] mock argument at round ${session.current_round}`)
    session.transcript.push(`[defendant] mock rebuttal at round ${session.current_round}`)
    ...
    this.recordEvent(session, 'turn_complete', 'engine', {
      round: session.current_round,
      phase: session.is_ready_for_adjudication ? 'ready_for_adjudication' : 'running',
    })
  }
  ...
  private adjudicate(session: MockSession): void {
    ...
    if (!session.is_ready_for_adjudication) {
      session.transcript.push('[judge] adjudication requested early; accepted in mock mode')
    }

    session.is_ready_for_adjudication = false
    session.is_finished = true
    session.winner = session.current_round % 2 === 0 ? 'plaintiff' : 'defendant'

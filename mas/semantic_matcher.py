...
class SemanticMatcher:
    ...
    def _get_embedding(self, text: str) -> List[float]:
        if text not in self._cache: self._cache[text] = self.embedding_func.embed_query(text)
        return self._cache[text]
    
    def find_match(self, query: str, candidates: List[Tuple[str, str]]) -> Optional[str]:

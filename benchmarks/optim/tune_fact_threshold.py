"""Tune `dedup.fact_threshold` using synthetic legal FACT-style pairs.

This script uses fully self-constructed data and the runtime embedding/cosine
stack. It follows F1+/Recall+ objective with coarse+fine threshold scan.
"""
...
from benchmarks.optim.generate_fact_dedup_dataset import generate_fact_dedup_dataset
...
def _score_rows(
    ...
    for row in rows:
        sim = _cosine_similarity(cache[row["existing_fact"]], cache[row["new_fact"]])
...
def _aggregate_scan_row(
    threshold: float, metrics_by_seed: List[BinaryMetrics]
) -> Dict[str, Any]:
...
def _score_key_cases(
    *,
    ...
    for row in key_cases:
        texts.append(row["existing_fact"])
        texts.append(row["new_fact"])
...
def _render_report(
    *,
    ...
    lines: List[str] = []
    lines.append("# Round 3 Tuning Report: fact_threshold")
    lines.append("")
    lines.append("## Scope and Constraints")
    lines.append("- Parameter: `mas/config.py` -> `dedup.fact_threshold`")
    lines.append(f"- Baseline: {baseline_threshold:.2f}")

    lines.append(
        "- Data policy: fully synthetic FACT-style AIGC text, no ES, no historical DB sampling"
    )

    lines.append("- Objective: F1+ primary, Recall+ tie-break")
    lines.append("")
    lines.append("## FACT Style Rule")

    lines.append(
        "- Follow extraction-style legal facts with explicit roles/time/actions/object slots."
    )

    lines.append("- Seed examples include:")
    lines.append("  - `2013年2月1日，原告(甲方)与被告(乙方)签订厂房租赁合同。`")

    lines.append(
        "  - `合同约定甲方将天津开发区第十三大街116号(出口加工区内)泰东工业厂房1栋、2栋出租给乙方。`"
    )

    lines.append("  - `乙方承租厂房的用途为生产、办公、存货。`")
    lines.append("")
    ...
    lines.append("- Rough scan: 0.70 to 0.99, step 0.03")
    lines.append("- Fine scan: around rough best +/-0.06, step 0.01")
    lines.append("- Selection: max F1+, tie-break Recall+, then macro F1 and stability")
    lines.append("")
    lines.append("## Selected Threshold")
    lines.append(f"- Recommended `fact_threshold`: **{tuned_threshold:.2f}**")
    ...
    lines.append("```diff")
    lines.append("-    fact_threshold: float = {:.2f}".format(baseline_threshold))
    lines.append("+    fact_threshold: float = {:.2f}".format(tuned_threshold))
    lines.append("```")
    lines.append("")
    lines.append("## Notes")

    lines.append(
        "- Scan details: `benchmarks/optim/artifacts/fact_threshold_scan_train.json`."
    )

    lines.append(
        "- Validation summary: `benchmarks/optim/artifacts/fact_threshold_valid_comparison.json`."
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--report-path",
        type=Path,
        default=Path("reports/round3_fact_threshold.md"),
    )
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        data_path = artifacts_dir / f"fact_dedup_seed{seed}.jsonl"
        scored_path = artifacts_dir / f"fact_similarity_seed{seed}.jsonl"
    ...
    rough_thresholds = threshold_range(0.70, 0.99, 0.03)
    rough_rows: List[Dict[str, Any]] = []

    for threshold in rough_thresholds:
        metrics = [evaluate_binary(train_by_seed[seed], threshold) for seed in seeds]
        rough_rows.append(_aggregate_scan_row(threshold, metrics))
    ...
    for threshold in scan_thresholds:
        metrics = [evaluate_binary(train_by_seed[seed], threshold) for seed in seeds]
        scan_rows.append(_aggregate_scan_row(threshold, metrics))
    ...
    _write_json(artifacts_dir / "fact_threshold_scan_train.json", scan_rows)

    _write_json(
        artifacts_dir / "fact_threshold_valid_comparison.json",
        {
            "baseline_threshold": args.baseline_threshold,
            "tuned_threshold": tuned_threshold,
    ...
    _write_json(artifacts_dir / "fact_key_cases.json", key_case_rows)

    _write_json(
        artifacts_dir / "fact_round3_summary.json",
        {
            "seeds": seeds,
    ...
    print("[fact-threshold-tuning] done")

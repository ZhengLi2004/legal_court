...
def test_full_cycle():
    ...
    if "盗窃罪需证明主观故意" in insights[0]:
        print("✅ Upward Traversal (Semantic Insight Retrieval) works.")
    else:
        print("❌ Upward Traversal failed.")
    ...
    # 检查 Plaintiff 的节点是否变为了 VALIDATED
    p_nodes = [d for n, d in saved_graph.graph.nodes(data=True) if d['agent_id'] == "plaintiff"]
    if p_nodes and p_nodes[0]['status'] == NodeStatus.VALIDATED:
        print("✅ BackProp worked (Status Updated).")
    else:
        print(f"❌ BackProp failed. Status: {[d.get('status') for d in p_nodes]}")
